...
alter table SCRUMIT_TRACKER add constraint FK_SCRUMIT_TRACKER_PROJECT foreign key (PROJECT_ID) references SCRUMIT_TASK(ID)^
alter table SCRUMIT_TRACKER add constraint FK_SCRUMIT_TRACKER_WORKFLOW foreign key (WORKFLOW_ID) references WFSTP_WORKFLOW(ID)^
alter table SCRUMIT_TRACKER add constraint FK_SCRUMIT_TRACKER_PERFORMER foreign key (PERFORMER_ID) references SEC_USER(ID)^
create unique index IDX_SCRUMIT_TRACKER_UK_NUMBER_ on SCRUMIT_TRACKER (NUMBER_) where DELETE_TS is null ^
create index IDX_SCRUMIT_TRACKER_PROJECT on SCRUMIT_TRACKER (PROJECT_ID)^
create index IDX_SCRUMIT_TRACKER_WORKFLOW on SCRUMIT_TRACKER (WORKFLOW_ID)^
create index IDX_SCRUMIT_TRACKER_PERFORMER on SCRUMIT_TRACKER (PERFORMER_ID)^
-- end SCRUMIT_TRACKER
-- begin SCRUMIT_CHAT
