...
-- end SCRUMIT_TEAM
-- begin SCRUMIT_TASK
alter table SCRUMIT_TASK add constraint FK_SCRUMIT_TASK_PARENT_BUG foreign key (PARENT_BUG_ID) references SCRUMIT_TRACKER(ID)^
...
alter table SCRUMIT_TASK add constraint FK_SCRUMIT_TASK_PERFORMER foreign key (PERFORMER_ID) references SEC_USER(ID)^
alter table SCRUMIT_TASK add constraint FK_SCRUMIT_TASK_TASK foreign key (TASK_ID) references SCRUMIT_TASK(ID)^
alter table SCRUMIT_TASK add constraint FK_SCRUMIT_TASK_TOP foreign key (TOP_ID) references SCRUMIT_TASK(ID)^
alter table SCRUMIT_TASK add constraint FK_SCRUMIT_TASK_SPRINT_BACKLOG foreign key (SPRINT_BACKLOG_ID) references SCRUMIT_SPRINT_BACKLOG(ID)^
create unique index IDX_SCRUMIT_TASK_UK_SHORTDESC on SCRUMIT_TASK (SHORTDESC) where DELETE_TS is null ^
create index IDX_SCRUMIT_TASK_PARENT_BUG on SCRUMIT_TASK (PARENT_BUG_ID)^
create index IDX_SCRUMIT_TASK_ESTIMATION on SCRUMIT_TASK (ESTIMATION_ID)^
create index IDX_SCRUMIT_TASK_PERFORMER on SCRUMIT_TASK (PERFORMER_ID)^
create index IDX_SCRUMIT_TASK_TASK on SCRUMIT_TASK (TASK_ID)^
create index IDX_SCRUMIT_TASK_TOP on SCRUMIT_TASK (TOP_ID)^
create index IDX_SCRUMIT_TASK_SPRINT_BACKLOG on SCRUMIT_TASK (SPRINT_BACKLOG_ID)^
-- end SCRUMIT_TASK
